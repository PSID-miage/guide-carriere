...
def load_cr_gd_dp_mapping() -> pd.DataFrame:
    """
    Charge la table d’association métier :  grand domaine (code_rome : libellé_grand_domaine)
...
# ----- Fonctions d'analyse et de graphiques -----

def plot_top_competences(liens: pd.DataFrame, competences: pd.DataFrame, top_n: int = 15):
    """
    Renvoie un DataFrame et un bar chart montrant les Top N compétences apparaissant
    dans le plus grand nombre de métiers.
    """
    counts = (
    ...
    fig = px.bar(
        ...
        labels={"libellé_competence": "Compétence"},
...
def plot_category_distribution(liens: pd.DataFrame, competences: pd.DataFrame):
    """
    Répartition des catégories de compétences (savoir-faire vs savoir-être) en
    comptant chaque lien métier–compétence.
    """
...
def plot_subcategory_distribution(competences: pd.DataFrame):
    """
    Répartition des sous-catégories (Technique, Technique expert, Transverse, etc.).
    """
...
def plot_ratio_transverse(liens: pd.DataFrame, competences: pd.DataFrame, metiers: pd.DataFrame, top_n: int = 10):
    """
    Calcule, pour chaque métier, la part de compétences de sous‑catégorie 'Transverse'
    et renvoie les Top N métiers les plus favorables à ces compétences.
    """
...
def compute_domain_skill_summary(liens: pd.DataFrame, domain_df: pd.DataFrame):
    """
    Calcule, pour chaque domaine, la moyenne du nombre de compétences par métier et
    le nombre de métiers présents dans ce domaine.
    """
...
def plot_domain_skill_summary(summary_df: pd.DataFrame):
    """
    Génère le bar chart montrant la moyenne de compétences par métier pour chaque domaine.
    """
    fig = px.bar(
        summary_df,
...
def prepare_domain_job_skill_breakdown(liens: pd.DataFrame, competences: pd.DataFrame, metiers: pd.DataFrame, domain_df: pd.DataFrame):
    """
    Prépare un DataFrame détaillant pour chaque domaine et chaque métier, le nombre de compétences
    par sous‑catégorie et le nombre de savoir‑être (cat_comp == 'Savoir-être professionnel').
    """
    merged = (
...
def plot_job_skill_breakdown(job_skill_counts: pd.DataFrame, selected_code_rome: str):
    """
    Génère un bar chart montrant la répartition des compétences par type pour un métier choisi.
    """
    ...
    fig = px.bar(
        ...
        color_discrete_sequence=["#8B4513", "#A0522D", "#CD853F", "#D2691E", "#8B4513"],
    )
    return fig