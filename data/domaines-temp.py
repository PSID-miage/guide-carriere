...
codes = pd.read_csv(codes_file, sep=",", encoding="utf-8")  # Update separator if needed
##print("Columns in 'codes':", codes.columns)  # Debugging: Check column names
#print(codes.head())  # Debugging: Check the first few rows of the DataFrame
...
appel = pd.read_csv(appel_file, sep=",", encoding="utf-8")  # Update separator if needed
appel.columns = appel.columns.str.strip()  # Clean column names
m18_codes = codes[mask]["code_rome"].tolist()
print(appel[appel["code_rome"].isin(m18_codes)][["code_rome", "libelle_appellation_long"]])